...
покупатель заходит на сайт, вводит валидные данные карты (номер 4444 4444 4444 4441, срок действия 08.2027, ФИО Ivan Ivanovich Vetrov, CVC 999), остальные данные тоже валидные. Нажать «Продолжить». 
...
покупатель заходит на сайт, выбирает "Купить в кредит", нажимает «Продолжитьу». 
...
1.3.4 При введении всех актуальных данных карты не возникает ошибок. 

1.4	Негативные сценарии

1.4.1 При покупке тура по дебетовой карте происходит ошибка со статусом DECLINED: 
клиент вводит актуальные данные карты, средств на карте достаточно, но происходит отказ: покупатель заходит на сайт, вводит валидные данные карты (номер 4444 4444 4444 4441, срок действия 08.2027, ФИО Ivan Ivanovich Vetrov, CVC 999), остальные данные тоже валидные. Нажимает «Продолжить».
Ожидаемый результат: Уведомление об отказе в проведении операции». Запись со статусом в БД «DECLINED».

1.4.2 При оформлении кредита на тур происходит отказ: 
нажимает «Купить в кредит». Вводит валидные данные. Нажимает «Продолжить». 
Ожидаемый результат: Уведомление об отказе Банком в выдаче кредита.
Запись со статусом в БД «DECLINED».

1.4.3 Все поля пустые, нет возможности отправить заявку: 
...
1.4.4 Невалидный номер карты, то есть менее 16 цифр: 
...
1.4.8 ФИО на кириллице, форма выдаёт ошибку: 
...
1.4.9 Пустое поле «Фамилия»: 
Ввести валидные данные без фамилии (номер карты 4444 4444 4444 4441, срок действия 08.2027, ФИО Ivan Ivanovich, CVC 999).  Нажать «Продолжить». 
...
1.4.10 Пустое поле «Имя»: 
Ввести валидные данные без имени (номер карты 4444 4444 4444 4441, срок действия 08.2027, ФИО Ivanovich Vetrov, CVC 999).  Нажать кнопку «Продолжить». 
